...
import Drawing.Shapes.coreElements.My3DRectangle;   // <-- ADD THIS
...
//		 MouseEvent event;
	...
	public void paintJPanelMousePressed(MouseEvent event) {
		...
		// user selected 3D rectangle
		else if (currentType.equals("3D Rectangle")) {                       // <-- ADD THIS
			currentShape = new My3DRectangle(event.getX(), event.getY(),    // <-- ADD THIS
					event.getX(), event.getY(), currentColor);              // <-- ADD THIS
		}                                                                     // <-- ADD THIS
